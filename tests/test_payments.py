...
def test_is_premium_returns_false_when_no_premium(mock_session_class: MagicMock) -> None:
    """User without any subscription should not be premium."""
    mock_db = MagicMock()
    mock_session_class.return_value = mock_db

    mock_user = MagicMock()
    mock_user.premium_until = None
    mock_user.premium_plus_until = None
...
@patch("app.payments.SessionLocal")
def test_is_premium_returns_true_when_only_plus_active(mock_session_class: MagicMock) -> None:
    """User with only premium_plus_until in future should be premium (any sub counts)."""
    mock_db = MagicMock()
    mock_session_class.return_value = mock_db

    mock_user = MagicMock()
    mock_user.premium_until = None
    mock_user.premium_plus_until = datetime.datetime.now(datetime.timezone.utc).replace(
        tzinfo=None
...
@patch("app.payments.SessionLocal")
def test_is_premium_plus_returns_true_when_active(mock_session_class: MagicMock) -> None:
    """User with future premium_plus_until should be premium_plus."""
    mock_db = MagicMock()
    mock_session_class.return_value = mock_db
    ...
    assert _is_premium_plus(12345) is True


@patch("app.payments.SessionLocal")
def test_is_premium_plus_returns_false_when_only_premium(mock_session_class: MagicMock) -> None:
    """User with only premium_until active should NOT be premium_plus."""
    mock_db = MagicMock()
    mock_session_class.return_value = mock_db
    ...
    assert _is_premium_plus(12345) is False
...
@patch("app.payments.SessionLocal")
def test_activate_premium_plus_sets_correct_date(mock_session_class: MagicMock) -> None:
    """Premium+ activation should set premium_plus_until to now + 30 days."""
    mock_db = MagicMock()
    mock_session_class.return_value = mock_db
    ...
    result = _activate_premium_plus(12345)
